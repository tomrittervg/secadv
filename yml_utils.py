...
class Advisory:
    def __init__(self, bugJSON, advisoryText):
        ...
        # cleanUpRealName(bugJSON['creator_details']['real_name'])
...
def getAdvisoryAttachment(bugid):
    link = "https://bugzilla.mozilla.org/rest/bug/" + \
        str(bugid) + "/attachment?api_key=" + APIKEY
    r = requests.get(link)
    ...
    for a in attachments:
        if a['description'] == "advisory.txt" and not a['is_obsolete']:
            if advisory is not None:
                raise Exception(
                    str(bugid) + " has two advisory.txt attachments")
...
def sanityCheckBugs(bugs, require_cves=False):
    ...
    for b in bugs:
        ...
        if require_cves and not b['alias']:
            eprint(
                bugid, "is missing a CVE identified. Please contact Tom Ritter (cc Dan Veditz) to have one assigned.")
            retvalue = False
